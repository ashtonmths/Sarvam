...
/** Collected and returned, so a caller with no console can report progress. */
let lines: string[] = [];
...
/**
 * Seeds the demo dataset into an organisation and reports what it wrote.
 *
 * Exported so the same work is reachable from a button as well as a script. A
 * hosted deployment has no shell, and the data this writes — the transcripts,
 * the commit history, the checkpoints — is what makes every other page worth
 * looking at, so leaving it script-only meant the product demoed as empty.
 *
 * Idempotent: each step clears its own rows before writing, so pressing the
 * button twice gives the same result rather than doubling everything.
 */
export async function seedDemoData(orgId: number): Promise<{ log: string[] }> {
  lines = [];

  await seedDocuments(orgId);
  const repoId = await seedChanges(orgId);
  await seedCheckpoints(orgId, repoId);
  await seedRationale(orgId);
  await seedMetrics(orgId);
  await seedIncident(orgId);
  await seedSlackScopes(orgId);
  await seedWorkflowFailure(orgId);

  appLog().info({ event: "demo_data_seeded", orgId }, "demo: dataset seeded");
  return { log: lines };
}
